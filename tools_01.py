"""Tools for cero-01.py — built on langchain_chroma.Chroma.

Zero dependencies on backend/core/. Each factory receives a
langchain_chroma.Chroma instance and returns a @tool-decorated function.
"""
...
def make_search_transcripts(store, top_k: int = 3):
    ...
    def search_transcripts(query: str) -> str:
        ...
        blocks.append(f"---\n{len(docs)} chunk(s) from {len(seen)} video(s)")
        return "\n\n".join(blocks)
